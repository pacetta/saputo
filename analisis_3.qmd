...
```{r}
# DATOS
pacman::p_load(rio, dplyr, ggplot2, stringr, tidyr, tables, car, grid, gridExtra, flextable, tidyverse, gtable, gt)
...
#display.brewer.all()
# Define a palette
mypalette4 <- brewer.pal(4, "Greys")
mypalette5 <- brewer.pal(5, "Greys")
mypalette6 <- brewer.pal(6, "Greys")
...
### Tamaño del tambo, en cantidad de vacas ordeño
...
### Edad de los encuestados y tamaño de tambo
...
### Provincia en la que se localiza el establecimiento
...
#chisq.test(datos5$Edad,datos5$Tamaño)
```
...
test7 <- datos9 %>% 
  ...
  summarise(n=n()) %>% 
  group_by(pregunta) %>% 
  summarise(test_result = fisher.test(Edad,Tamaño)$p.value)
```

# Análisis preguntas 5 a 9 por Provincia
...
lab_pcias <-  datos_pcia %>% 
  group_by(PCIAS, Tamaño) %>% 
  summarise(n = n()) %>% 
  transmute(Tamaño, "%" = n /sum(n)*100)

lab_pcias %>% 
  ggplot(.) +
  aes(x = `PCIAS`, y= `%`, fill = `Tamaño`) +
  geom_col(position = 'dodge') +
  labs (y= "%" , x = "", fill = "",
        caption = paste0("n = ", length(datos$`Marca temporal`)))  +
    tema +  geom_text(aes(label = paste(round(lab_pcias$`%`,1),"%")),
...
gpcia <- datos_pcia %>% 
  ...
  aes(x = `5. Para Ud., la conservación del medio ambiente es:`, 
      y =..count../sum(..count..)*100) +
  geom_bar() +
  facet_grid( `4. ¿Qué tamaño tiene su tambo, en cantidad de vacas ordeño?` ~ PCIAS + `2.  ¿En qué rango de edad se encuentra?`) +
  labs (y= "%" , x = "Opinión", title = titulo5) + 
tema +   geom_text(aes(label = paste(round(..count../sum(..count..)*100,1),"%")), 
                     stat = "count", vjust = 0, hjust = 0) +
  coord_flip() +  scale_x_discrete(labels = function(x) str_wrap(x, 25))
...
graf <- datos6 %>% 
  ...
  labs(y = "%" , fill = "", x = "Percepción", 
    ...
    tema +
    coord_flip() +  scale_fill_grey() + scale_x_discrete(labels = function(x) str_wrap(x, 20)) ))
...
```{r, fig.height= 9}
datos10 <- pivot_longer(datos_pcia, cols = c(34), names_to = "pregunta") %>% 
...
datos10_ <- datos10[ , 36:39] %>% 
  group_by(PCIAS, Edad, Tamaño) %>% 
  summarise(n = n())%>% 
  transmute(Tamaño, "% por provincia" = n /sum(n)*100)
...
# Mostrar la tabla de presentación
tabla_presentacion
...
graf <- datos10 %>% 
  filter(`3. ¿Podría seleccionar en qué provincia se localiza el establecimiento lechero?` == c("Córdoba", "Buenos Aires", "Santa Fe")) %>%
  filter(`2.  ¿En qué rango de edad se encuentra?` %in% c("26 a 45","46 a 65")) %>%   aes(fill = `3. ¿Podría seleccionar en qué provincia se localiza el establecimiento lechero?`, x = pregunta, 
      y = ..count../sum(..count..)*100) +
  geom_bar() +
#  facet_grid(`2.  ¿En qué rango de edad se encuentra?`~ `4. ¿Qué tamaño tiene su tambo, en cantidad de vacas ordeño?`) +
  labs(y = "%" , fill = "", x = "", 
